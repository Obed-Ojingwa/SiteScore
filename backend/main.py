...
def capture_lead(short_id: str, email: str) -> None:
    ...
    try:
        ...
        # Any duplicate-key style violation just means this email is already on
        # file (possibly against a different constraint shape than we expect on
        # the live table) -- treat that as success rather than failing the unlock.
        if "duplicate key" in str(exc).lower() or "unique constraint" in str(exc).lower():
            logger.info("Lead already exists, continuing: %s", email)
            return
