    ...
    {!audit ? <section className="hero shell">
      ...
      <section className="issues-panel">
        <div className="panel-heading">
          <div>
            <div className="eyebrow dark"><span className="eyebrow-line" /> PRIORITY FIXES</div>
            <h2>What to look at first.</h2>
          </div>
          {isUnlocked ? <button className="pdf-button" onClick={downloadPdf}><Download size={14} /> Download PDF</button> : <span className="issue-count">{audit.issues.length} highest-impact issues</span>}
        </div>
        {isUnlocked ? (
          <div className="issue-list">
            {audit.issues.length > 0 ? audit.issues.map((issue, index) => (
              <div className="issue" key={`${issue.title}-${index}`}>
                <span className="issue-number">0{index + 1}</span>
                <div>
                  <div className="issue-meta">
                    <span>{issue.category}</span><b>{issue.lost_points} pts at risk</b>
                  </div>
                  <h3>{issue.title}</h3>
                  <p>{issue.explanation}</p>
                </div>
              </div>
            )) : <p>No priority issues were found for this audit.</p>}
          </div>
        ) : null}
        {/* <form className="email-gate" onSubmit={unlockReport}><div><strong>Unlock the fix list</strong><p>Get the plain-English actions behind this score, plus a downloadable PDF.</p></div><div className="email-gate-form"><input type="email" required value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@company.com" aria-label="Email address" /><button type="submit" disabled={isSubmittingEmail}>{isSubmittingEmail ? <LoaderCircle className="spin" size={15} /> : <>Unlock report <ArrowUpRight size={15} /></>}</button></div></form> */}
      </section>
